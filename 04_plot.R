...
by_field_df <- read.csv("papers_by_field.csv", row.names = 1)
...
q <- ggplot(by_field_df, aes(y = field, x = paper_count)) +
  ...
  labs(
    title = "Number of papers citing selected dimensionality reduction methods, 2013-2023",
    x = "Number of Papers",
    y = "Field"
  )
ggsave("papers_by_field.png", plot = q, width = 10, height = 7, dpi = 300)


# Plot papers by field and cited method
by_field_method_df <- read.csv("papers_by_field_and_method.csv", row.names = 1)
...
ggsave("papers_by_field_and_method.png", plot = q, width = 10, height = 15, dpi = 300)

# Plot papers by venue
by_venue_df <- read.csv("papers_by_venue.csv", row.names = 1)
...
q <- ggplot(by_venue_df, aes(y = venue, x = paper_count)) +
  geom_bar(stat = "identity") +
  labs(
    title = "Number of papers citing selected dimensionality reduction methods, 2013-2023, top 50 venues",
    x = "Number of Papers",
    y = "Venue"
  )
ggsave("papers_by_venue.png", plot = q, width = 13, height = 30, dpi = 300)

# Plot papers by venue and cited method
by_field_method_df <- read.csv("papers_by_venue_and_method.csv", row.names = 1)
...
ggsave("papers_by_venue_and_method.png", plot = q, width = 10, height = 15, dpi = 300)
